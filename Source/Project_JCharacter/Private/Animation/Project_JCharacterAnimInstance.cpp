...
FBoolProperty* GetMotionMatchingShouldSearchProperty()
{
	static FBoolProperty* Property = FindFProperty<FBoolProperty>(
		FAnimNode_MotionMatching::StaticStruct(),
		TEXT("bShouldSearch"));
	return Property;
}

void SetMotionMatchingShouldSearch(FAnimNode_MotionMatching& Node, bool bShouldSearch)
{
	if (FBoolProperty* Property = GetMotionMatchingShouldSearchProperty())
	{
		Property->SetPropertyValue_InContainer(&Node, bShouldSearch);
	}
}

bool GetMotionMatchingShouldSearch(const FAnimNode_MotionMatching& Node)
{
	if (const FBoolProperty* Property = GetMotionMatchingShouldSearchProperty())
	{
...
void FProject_JCharacterAnimInstanceProxy::ApplyMotionMatchingSearchPolicy()
{
	...
	SetMotionMatchingShouldSearch(NativeMotionMatchingNode, bShouldSearchEveryUpdate);
	...
	SetMotionMatchingSearchThrottleTime(
		...
		bShouldSearchEveryUpdate || bNativeDatabaseChanged
	...
	{
		if (const FAnimNode_MotionMatching* MotionMatchingNode = GetNodeFromIndex<FAnimNode_MotionMatching>(NodeIndex))
		{
			...
			SetMotionMatchingShouldSearch(
				*const_cast<FAnimNode_MotionMatching*>(MotionMatchingNode),
				bShouldSearchEveryUpdate);
			SetMotionMatchingSearchThrottleTime(
				*const_cast<FAnimNode_MotionMatching*>(MotionMatchingNode),
				bShouldSearchEveryUpdate || bDatabaseChanged
	...
	for (int32 NodeIndex = 0; NodeIndex < AnimNodeProperties.Num(); ++NodeIndex)
	{
		...
		UE_LOG(
			...
			*GetNameSafe(GetAnimInstanceObject()),
			...
			GetMotionMatchingShouldSearch(*MotionMatchingNode) ? TEXT("true") : TEXT("false"),
			GetMotionMatchingSearchThrottleTime(*MotionMatchingNode),
