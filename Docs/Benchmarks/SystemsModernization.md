...
> 2026-09-13 검증 도구 정정: `Measure-CrowdE.ps1`의 단일 `-NullRHI` 인자가 PowerShell에서 문자별로 펼쳐지는 경로를 확인해 `string[]`로 고정했다. 이 스크립트의 과거 non-rendered 실행은 manifest의 `rendered=false`만으로 NullRHI 사용을 단정할 수 없으며, 원본 로그의 실제 명령행/RHI를 확인해야 한다. 보관된 측정 수치는 변경하지 않았다. 이번 내부 리팩터링은 새 성능 향상률을 제시하지 않는다. [변경·검증 범위](../Architecture/Runtime/Internal_Refinement_2026-09-13.md)
