...
* **궤적 컴포넌트 Tick-Free (Motion Matching Trajectory Update Path 이동)**: `Project_JMotionMatchingTrajectoryComponent`의 자체 컴포넌트 틱(`bCanEverTick`)을 끄고, 궤적 업데이트 호출을 `UProject_JCharacterAnimInstance::NativeUpdateAnimation`으로 이관했습니다. 이를 통해 언리얼 엔진의 URO(Update Rate Optimization)와 Significance Manager의 혜택을 온전히 받아 멀리 있는 캐릭터의 연산량을 크게 줄였습니다.
...
### [Tick & 동기화 구조]
* **Tick & Significance Manager 활용**: `AProject_JBaseCharacter`에서 불필요한 액터 Tick을 최대한 끄고(입력/상태 갱신이 필수인 PlayerCharacter 등 예외 제외), 거리에 따라 Tick 주기를 조절하는 구조가 반영되어 있습니다.
...
* **해결**: `Project_JNPCCharacter.cpp` 생성자에 코드를 추가하여 복제 모드를 `Minimal`로 강제 변경했습니다. 이로써 몬스터 밀집 구역에서 GameplayEffect의 세부 복제를 줄여 네트워킹 부하를 완화했습니다.
